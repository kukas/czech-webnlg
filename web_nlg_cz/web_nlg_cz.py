...
_FILE_PATHS = {
    "release_v3.0_en_cs": {
        "train": [f"release_v3.0/en_cs/train/{i}triples/" for i in range(1, 8)],
        "validation": [f"release_v3.0/en_cs/dev/{i}triples/" for i in range(1, 8)],
        "test": ["release_v3.0/en_cs/test/"],
    },
    "release_v3.0_ru_cs": {
        "train": [f"release_v3.0/ru_cs/train/{i}triples/" for i in range(1, 8)],
        "validation": [f"release_v3.0/ru_cs/dev/{i}triples/" for i in range(1, 8)],
        "test": ["release_v3.0/ru_cs/test/"],
    },
}
...
def parse_entry(entry):
    ...
    res["original_triple_sets"] = [
        {"otriple_set": otriple_set["otriple"]} for otriple_set in otriple_set_list
    ]
    mtriple_set_list = entry["modifiedtripleset"]
    res["modified_triple_sets"] = [
        {"mtriple_set": mtriple_set["mtriple"]} for mtriple_set in mtriple_set_list
    ]
    ...
    res["lex"] = {
        ...
        "source": [ex.get("source", "") for ex in entry.get("lex", [])],
    }
    ...
    if dbpedia_links:
        res["dbpedia_links"] = [
            dbpedia_link["text"] for dbpedia_link in dbpedia_links[0]["dbpedialink"]
        ]
    else:
        res["dbpedia_links"] = []
    links = entry["links"]
...
class WebNlg(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        datasets.BuilderConfig(
            name="release_v3.0_en_cs",
            description="WebNLG+ data used in the WebNLG challenge 2020. English with Czech translations.",
        ),
        datasets.BuilderConfig(
            name="release_v3.0_ru_cs",
            description="WebNLG+ data used in the WebNLG challenge 2020. Russian with Czech translations.",
        ),
    ]
        ...
        for xml_location in filedirs:
            for xml_file in sorted(glob(pjoin(xml_location, "*.xml"))):
                ...
                if (
                    ...
                    or "release_v3.0/en_cs/test" in xml_file_path_with_slashes
                    or "release_v3.0/ru_cs/test" in xml_file_path_with_slashes
                ):
